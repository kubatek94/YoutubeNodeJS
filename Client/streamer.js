(function ()
{
	...
	scriptsLoader
		...
		.onComplete(function(){
			...
			var socket = io((window.YOUTUBE_NODE_JS_HOST + ":" + window.YOUTUBE_NODE_JS_PORT) + "/stream");
			var youtubeStreamerClient = new YoutubeStreamerClient(socket);
			...
			youtubeStreamerClient
				...
				.onProgress(new YoutubeCallback(function (args)
				{
					...
					progressWindow.setValue(Math.floor(((args.size / this.youtubeStreamerClientFile.fileSize) * 100)));
				}, youtubeStreamerClient))
			...
			youtubeStreamerClient.stream(url);
		});
	...
	scriptsLoader.addScript((window.YOUTUBE_NODE_JS_HOST + ":" + window.YOUTUBE_NODE_JS_PORT) + "/Client/YoutubeStreamerClient.js?type=text/javascript");
